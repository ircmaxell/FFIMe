...
class test {
    const SOFILE = \'%s\';
    const HEADER_DEF = \'void set_callback(void(*test_callback)(int param1, int param2));
\';
    private FFI $ffi;
...
class void_ptr_ptr_ptr implements itest {
    ...
    public function tearDown(): void {
        @unlink(__DIR__ . '/callbacksTest.result.php');
    }

    /**
     * @textdox Test callbacks
     */
    public function testCodeGen() {
        $this->lib->include(__DIR__ . '/callbacksTest.h');
        $this->lib->codeGen("test\\test", __DIR__ . '/callbacksTest.result.php');
        $this->assertFileExists(__DIR__ . '/callbacksTest.result.php');
        $this->assertStringMatchesFormat(self::EXPECTED, trim(file_get_contents(__DIR__ . '/callbacksTest.result.php')));
    }
}