...
class size_t_ptr implements itest, itest_ptr, \ArrayAccess {
    ...
    public static function array(int $size = 1): self { return test::makeArray(self::class, $size); }
    /** @return size_t[] */ public function toArray(?int $length = null): array { $ret = []; if ($length === null) { $i = 0; while ($cur = $this->data[$i++]) { $ret[] = $cur; } } else { for ($i = 0; $i < $length; ++$i) { $ret[] = $this->data[$i]; } } return $ret; }
